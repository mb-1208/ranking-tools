...
const getRarity = () => {
  // read json data
  const rawdata = fs.readFileSync(`${basePath}/build/project_metadata/_metadata_(PROJECT_NAME).json`);
...
function processRarity(nfts) {
  ...
  fs.writeFileSync(`${basePath}/build/rarity/_metadata_(PROJECT_NAME).json`, JSON.stringify(nfts, null, 2));
}
