...
const getRarity = () => {
  // read json data
  const rawdata = fs.readFileSync(`${basePath}/testrarityparas.json`);
...
function processRarity(nfts) {
  ...
  fs.writeFileSync(`${basePath}/build/rarity/_metadata_with_rarity_api.json`, JSON.stringify(nfts, null, 2));
}
