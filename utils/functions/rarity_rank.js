...
(async () => {
  try {
    ...
    const rawdata = fs.readFileSync(
      `${basePath}/build/rarity/_metadata_with_rarity.json`
    );
    ...
    if (choice === "1") {
      ...
      fs.writeFileSync(
        `${basePath}/build/json/_metadata_with_rarity_rank.json`,
        JSON.stringify(
          topNfts.map(({ rank, total_rarity_score, name }) => {
            return {
              name,
      ...
      fs.writeFileSync(
        `${basePath}/build/json/_metadata_with_rarity_rank(choice).json`,
        JSON.stringify(
          {
            name: nft.name,
            rank: nft.rank,
